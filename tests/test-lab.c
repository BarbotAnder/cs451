...
void test_buddy_realloc_behavior(void)
{
  ...
  assert(ptr != NULL);
  strcpy((char *)ptr, "teststring");

  void *bigger = buddy_realloc(&pool, ptr, 256);
  assert(bigger != NULL);
  assert(strcmp((char *)bigger, "teststring") == 0);

  void *smaller = buddy_realloc(&pool, bigger, 32);
  assert(smaller != NULL);
  assert(strcmp((char *)smaller, "teststring") == 0);
